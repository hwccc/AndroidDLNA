...
public class DlnaUtils {
	...
	public static int parseSeekTime(String value, String data) throws Exception {
		int seekPos = 0;
		...
		if (PlatinumReflection.MEDIA_SEEK_TIME_TYPE_REL_TIME.equals(timetype)){
			...
			log.e("timetype = " + value + ", position = " + data);
		}
		return seekPos;
	}
	...
	public static boolean isAudioItem(DlnaMediaModel item){
		...
		if (objectClass.contains(DLNA_OBJECTCLASS_MUSICID) || DLNA_OBJECTCLASS_MUSICID.contains(objectClass)){
			return true;
		}
	...
	public static boolean isVideoItem(DlnaMediaModel item){
		String objectClass = item.getObjectClass();
		if (objectClass.contains(DLNA_OBJECTCLASS_VIDEOID) || DLNA_OBJECTCLASS_VIDEOID.contains(objectClass)){
			return true;
		}
		return false;
	}
	...
	public static boolean isImageItem(DlnaMediaModel item){
		String objectClass = item.getObjectClass();
		if (objectClass.contains(DLNA_OBJECTCLASS_PHOTOID) || DLNA_OBJECTCLASS_PHOTOID.contains(objectClass)){
			return true;
		}
		return false;
	}
